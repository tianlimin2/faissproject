...
def load_eval_data(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        eval_data = [row for row in reader]
...
def main():
    ...
    eval_vector_path = os.path.join(vectors_folder_path, "test_question_vectors.npy")
    ...
    # 尝试加载评估问题的向量
    eval_questions = df_eval['sentence'].tolist()
    ...
    # 打开文件以保存结果
    with open("results_test.json", "w", encoding="utf-8") as f:  # 保存到新的文件中
        for i, eval_question_vector in enumerate(eval_question_vectors[:2000]):
            ...
            # 打印结果
            result_json = {
                "symptom_description": eval_questions[i],
                "expected_department": label,
                "request": similar_questions,
                #"检索时间": round(end_time - start_time, 4)
            }
            print(json.dumps(result_json, ensure_ascii=False, indent=4))  # 打印到控制台
            results.append(result_json)  # 添加到结果列表中
            ...
            # })
        # 将总检索时间也保存到文件中
        json.dump(results, f, ensure_ascii=False, indent=4)  # 将结果写入文件中
